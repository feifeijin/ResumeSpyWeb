    ...
    <div class="legal-page">
      <div class="legal-inner">
        ...
        <section v-for="key in sections" :key="key" class="legal-section">
          <h2 class="section-heading">{{ t(`legal.privacy.${key}.heading`) }}</h2>
          <p class="section-body">{{ t(`legal.privacy.${key}.body`) }}</p>
        </section>
      </div>
    </div>
  </div>
...
const sections = ['intro', 'collection', 'stripe', 'retention', 'contact'] as const

let metaEl: HTMLMetaElement | null = null

onMounted(() => {
  document.title = `${t('legal.privacy.title')} — ResumeSpy`
  metaEl = document.createElement('meta')
  metaEl.setAttribute('name', 'description')
  metaEl.setAttribute('content', t('legal.privacy.metaDesc'))
